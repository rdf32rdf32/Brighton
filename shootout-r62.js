// Albion Fan Hub r62 penalty game
(() => {
  "use strict";
  ...
  // r62: Brighton can deliberately aim narrowly wide or high. The visible target is
  ...
  const albionTakers = window.ALBION_DATA_R62?.penaltyTakers || [];
  const palaceTakers = window.ALBION_DATA_R62?.palaceTakers || [];
  ...
  async function prepareAlbionKick() {
    ...
    if (panenka) panenka.disabled = false;
        $("stageInstruction").textContent = "Drag to aim, release to shoot";
    setStatus("Pick your spot", "Aim up to 3% beyond the posts or crossbar, but outside placement can miss.");
  }
  ...
  async function beginPalacePenalty() {
    ...
    const anticipatedDive = state.pendingDive && performance.now() - state.pendingDive.time < 5000 ? state.pendingDive : null;
    stage.classList.remove("is-locked");
    stage.classList.add("is-save-window");
    $("stageInstruction").textContent = matchMedia(MOBILE_PENALTY_QUERY).matches ? "Swipe or choose one of nine save zones" : "Move or swipe towards the shot — early movement counts";
    setStatus("Read the final stride","Use the body shape as a clue, then commit. An early wrong guess cannot be reversed.");
    if (anticipatedDive) {
      window.setTimeout(() => {
    ...
    if (token !== state.sequence) return;
    cue.hidden = false;
    setStatus("React now", matchMedia(MOBILE_PENALTY_QUERY).matches ? "Swipe or tap a high, middle or low save zone." : "Move or swipe towards the shot. Choosing the correct side is strongly rewarded.");
  ...
  function previewKeeper(point) {
    if (!["palace-prep", "palace-run", "save"].includes(state.phase) || state.userDive) return;
  ...
  stage.addEventListener("keydown", (event) => {
    ...
    } else if (event.key.toLowerCase() === "p" && state.phase === "albion-aim") setShotStyle(state.shotStyle === "panenka" ? "normal" : "panenka");
    else handled = false;
