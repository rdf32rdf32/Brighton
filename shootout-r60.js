// Albion Fan Hub r60 penalty game
(() => {
  "use strict";
  ...
  const albionTakers = window.ALBION_DATA_R60?.penaltyTakers || [];
  const palaceTakers = window.ALBION_DATA_R60?.palaceTakers || [];
  ...
  async function prepareAlbionKick() {
    ...
    setStatus("Pick your spot", "Aim and release. Gesture speed and the taker shape the shot.");
  }
  ...
  stage.addEventListener("pointerup", (event) => {
    ...
    if (state.phase === "albion-aim" && state.aimPointerActive) {
      ...
      const aimPoint = aimPointForPointer(point, touchLike);
      setReticle(aimPoint.x, aimPoint.y);
      const shot={...state.aim};state.aimGesture=captureAimGesture(state.pointerStart,{clientX:event.clientX,clientY:event.clientY,time:performance.now()});clearPointerTracking();takeAlbionPenalty(shot);
      return;
    }
  ...
  stage.addEventListener("touchend", (event) => {
    ...
    if (touch) {
      ...
      const mapped = distance > 18 ? swipeGoalPoint(fallbackTouchStart, touch, mobileTapGoalPoint(touch)) : mobileTapGoalPoint(touch);
      if (state.phase === "albion-aim") {
        setReticle(point.x,point.y);state.aimGesture=captureAimGesture(fallbackTouchStart,{clientX:touch.clientX,clientY:touch.clientY,time:performance.now()});takeAlbionPenalty({...state.aim});
      } else if (state.phase === "save" && state.reactionOpen && !state.userDive) {
