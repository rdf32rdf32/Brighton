// Albion Fan Hub r79 penalty game — more natural goalkeeper dive mechanics
(() => {
  "use strict";
  ...
  stage.addEventListener("pointerdown", (event) => {
    ...
    if (!savingPhase && state.phase !== "albion-aim") {
      if (state.phase === "palace-prep" || state.phase === "palace-run") {
        ...
        $("stageInstruction").textContent = "Wait — or gamble early";
        setStatus("Read the run-up", "Wait for React now, or deliberately flick/click to commit early.");
      }
      return;
    }
    event.preventDefault();
