// Albion Fan Hub r59 penalty game
(() => {
  "use strict";
  ...
  const shotStylePlaced = $("shotStylePlaced");
  const shotStyleDriven = $("shotStyleDriven");
  const shotPower = $("shotPower");
  const shotPowerValue = $("shotPowerValue");
  ...
  const GAME = Object.freeze({
    ...
    diveAssist: 0.88,
    sameSideBonus: 0.29,
    scoringDifficultyIncrease: 0.065,
    contactDecisionDelay: 285,
    edgeAccuracyPenalty: 0.0055,
  });

  const albionTakers = window.ALBION_DATA_R59?.penaltyTakers || [];
  const palaceTakers = window.ALBION_DATA_R59?.palaceTakers || [];
  ...
  function setShotStyle(style = "placed") {
    const selected = ["placed", "driven", "panenka"].includes(style) ? style : "placed";
    state.shotStyle = selected;
    [shotStylePlaced, shotStyleDriven, panenka].forEach((button) => {
      if (!button) return;
      const active = button.dataset.shotStyle === selected;
      button.classList.toggle("active", active);
      button.setAttribute("aria-pressed", String(active));
    });
    if (shotPower) shotPower.disabled = selected === "panenka";
  }

  function selectedPower() {
    const value = Number(shotPower?.value || 78) / 100;
    return clamp(value, .45, 1);
  }
  ...
  function setReticle(nx, ny) {
    syncGoalBox();
    state.aim.x = clamp(nx, 0.04, 0.96);
    state.aim.y = clamp(ny, 0.05, 0.95);
  ...
  async function keeperBottleRoutine(token) {
    ...
    const duration = reducedMotion() ? 260 : (state.palaceKicks === 0 ? 2600 : 900);
    const body = keeper.querySelector('.keeper-body-group');
    const head = keeper.querySelector('.keeper-head-group');
    const leftArm = keeper.querySelector('.keeper-arm-left');
    const rightArm = keeper.querySelector('.keeper-arm-right');
  ...
  async function prepareAlbionKick() {
    ...
    [panenka, shotStylePlaced, shotStyleDriven, shotPower].forEach((control) => { if (control) control.disabled = true; });
    panenka.closest(".shot-style-controls")?.removeAttribute("hidden");
    ...
    [panenka, shotStylePlaced, shotStyleDriven, shotPower].forEach((control) => { if (control) control.disabled = false; });
    if (state.shotStyle === "panenka" && shotPower) shotPower.disabled = true;
  ...
  async function takeAlbionPenalty(aim) {
    ...
    const player = takerAt("albion", state.albionKicks);
    applyTakerPose(player, state.albionKicks);
    const isPanenka = state.shotStyle === "panenka";
    const userPower = selectedPower();
    const selectedStyle = isPanenka ? "panenka" : state.shotStyle;
    ...
    const styleSpread = selectedStyle === "driven" ? 1.22 : selectedStyle === "placed" ? .9 : 1;
    const powerSpread = userPower > .9 ? 1 + (userPower - .9) * 2.6 : userPower < .58 ? 1.12 : 1;
    ...
    const extraReadChance = settings.scoringDifficultyIncrease * (1.12 - userPower * .42) * (selectedStyle === "placed" ? 1 : .82);
  ...
  async function beginPalacePenalty() {
    ...
    if (token !== state.sequence) return;
    cue.hidden = false;
    setStatus("React now", matchMedia(MOBILE_PENALTY_QUERY).matches ? "Swipe anywhere on the pitch or tap left, centre or right." : "Move or swipe towards the shot. Choosing the correct side is strongly rewarded.");
  ...
  stage.addEventListener("pointerup", (event) => {
    ...
    if (state.phase === "palace-prep" || state.phase === "palace-run") {
      event.preventDefault();
      state.pendingDive = { point: swipePoint, time: performance.now(), source: distance > 18 ? (touchLike ? "early-swipe" : "early-mouse") : (touchLike ? "early-tap" : "early-click") };
  ...
  stage.addEventListener("keydown", (event) => {
    ...
    } else if (event.key.toLowerCase() === "p" && state.phase === "albion-aim") setShotStyle(state.shotStyle === "panenka" ? "placed" : "panenka");
    else handled = false;
  ...
  [shotStylePlaced, shotStyleDriven, panenka].forEach((button) => button?.addEventListener("click", () => {
    if (state.phase !== "albion-aim") return;
    setShotStyle(button.dataset.shotStyle);
  }));
  shotPower?.addEventListener("input", () => { if (shotPowerValue) shotPowerValue.textContent = `${shotPower.value}%`; });
  ...
  soundButton.setAttribute("aria-pressed", String(state.sound));
  setShotStyle("placed");
  if (shotPowerValue && shotPower) shotPowerValue.textContent = `${shotPower.value}%`;
  setReticle(0.5, 0.5);
  renderRecord();
