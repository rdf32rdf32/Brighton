...
/* ===== consolidated from app-r66.js ===== */
/* ===== Albion Fan Hub r76 application bundle ===== */
  ...
  seasonDatabase: {
    "2026/27": {
      ...
      results: [
        ...
        { opponent: "Coventry City", venue: "A", score: "0–5", outcome: "W", date: "13 Sep 2026" },
        { opponent: "Arsenal", venue: "H", score: "3–0", outcome: "W", date: "19 Sep 2026" },
      ],
    },
  },
  freshness: {
    fixtures: "25 September 2026",
    squad: "16 September 2026",
...
(() => {
  ...
  function initTour() {
    ...
    const steps = [
      ["prototype", "Welcome to Albion Fan Hub", "Start with the purpose of this independent supporter-built project."],
...
(() => {
  ...
  // r66: one fixture list drives the hero, countdown, sticky bar, Match Centre and predictor.
  // The three-hour match window prevents the site switching opponents at kick-off.
  const FIXTURE_MONTHS = Object.freeze({ Jan:0, Feb:1, Mar:2, Apr:3, May:4, Jun:5, Jul:6, Aug:7, Sep:8, Oct:9, Nov:10, Dec:11 });
  const lastSunday = (year, monthIndex) => {
    const date = new Date(Date.UTC(year, monthIndex + 1, 0));
    return date.getUTCDate() - date.getUTCDay();
  };
  function fixtureTimestamp(fixture) {
    if (!fixture?.date) return Number.NaN;
    ...
    const bstEnd = Date.UTC(year, 9, lastSunday(year, 9), 1, 0);
    const londonOffsetMs = localGuess >= bstStart && localGuess < bstEnd ? 60 * 60 * 1000 : 0;
    return localGuess - londonOffsetMs;
  }
  function fixtureToMatch(fixture) {
    ...
    const londonDate = new Intl.DateTimeFormat("en-GB", { weekday:"long", day:"numeric", month:"long", year:"numeric", timeZone:"Europe/London" }).format(kickoff);
    const shortDate = new Intl.DateTimeFormat("en-GB", { day:"numeric", month:"short", timeZone:"Europe/London" }).format(kickoff);
    return {
      ...fixture,
      dateLong: londonDate,
      dateShort: shortDate,
  ...
  let MATCH = selectActiveMatch() || C.nextMatch || {
    ...
    dateLong: "Fixture to be confirmed",
    dateShort: "TBC",
    time: "TBC",
    venue: "TBC",
    venueCode: "H",
  ...
  function matchConfiguration() {
    ...
    $("stickyMatchDetail").textContent = `${MATCH.dateShort} · ${MATCH.time || "TBC"} · ${home ? "Amex" : MATCH.venue}`;
    ...
    const predictionClosed = matchGap <= 0;
    const savePrediction = $("savePrediction");
    if (savePrediction) {
      savePrediction.disabled = predictionClosed;
      savePrediction.setAttribute("aria-disabled", String(predictionClosed));
      savePrediction.title = predictionClosed ? "Predictions close at kick-off" : "Save this match prediction";
    }
    ["homeScore","awayScore","firstScorer","motm"].forEach((id) => { if ($(id)) $(id).disabled = predictionClosed; });
    const predictorCard = $("predictor");
    predictorCard?.classList.toggle("prediction-closed", predictionClosed);
  }

  function refreshActiveMatch({ force = false } = {}) {
    const selected = selectActiveMatch();
    if (!selected) {
      const noFixture = $("countdown");
      if (noFixture) noFixture.textContent = "Next fixture to be confirmed";
      return;
    }
    const changed = selected.dateISO !== MATCH.dateISO || selected.opponent !== MATCH.opponent || selected.venueCode !== MATCH.venueCode;
    if (changed || force) {
      MATCH = selected;
  ...
  function renderFixtures() {
    ...
    const fixtures = (C.fixtures || []).filter(
      (fixture) =>
        ...
        (month === "all" || fixture.date.slice(fixture.date.indexOf(" ") + 1) === month) &&
  ...
  function renderFixtureHighlights() {
    ...
    $("nextHomeDate").textContent = home ? `${home.date}${home.time ? ` · ${home.time}` : " · Time TBC"} · Amex Stadium` : "";
    $("nextAwayFixture").textContent = away ? `${away.opponent} v Albion` : "To be confirmed";
    $("nextAwayDate").textContent = away ? `${away.date}${away.time ? ` · ${away.time}` : " · Time TBC"} · Away` : "";
  }
  ...
  function predictor() {
    ...
    $("savePrediction").addEventListener("click", () => {
      const scoreText = MATCH.venueCode === "A"
        ? `${MATCH.opponent} ${$("awayScore").value}-${$("homeScore").value} Albion`
        : `Albion ${$("homeScore").value}-${$("awayScore").value} ${MATCH.opponent}`;
      const text = `${scoreText} · First scorer: ${$("firstScorer").value} · Player of the match: ${$("motm").value}`;
  ...
  function siteExperience() {
    ...
    const revealSearchTarget = (target) => {
      ...
      const matchPanel = target.closest?.(".match-view-panel");
      if (matchPanel?.id) window.AlbionActivateMatchView?.(matchPanel.id);
      ...
      if (hiddenParent && !hiddenParent.matches(".story-panel,.travel-panel,.record-panel,.match-view-panel")) hiddenParent.hidden = false;
    };
  ...
  refreshActiveMatch({ force: true });
  setInterval(() => refreshActiveMatch(), 60000);
  document.addEventListener("visibilitychange", () => { if (!document.hidden) refreshActiveMatch({ force: true }); });
  renderSquad();
  initXI();
  initFixtureMonths();
  renderFixtures();
...
(() => {
  ...
  function diagnostics() {
    const run = () => {
      ...
      const activeNames = (window.ALBION_DATA_R66?.squad || []).filter((player) => player.active !== false).map((player) => player.name);
      const duplicates = activeNames.filter((name, index) => activeNames.indexOf(name) !== index);
      const penaltyNames = window.ALBION_DATA_R66?.penaltyTakers?.map((player) => player.name) || [];
...
(() => {
  ...
  function refreshMatchCentre() {
    MATCH = C.nextMatch || MATCH;
    updateMatchDetails();
    matchCountdown();
  }
  refreshMatchCentre();
...
(() => {
  ...
  if (stage && ready) {
    ...
    const zones = [
      ['High left',.17,.18,'↖'],['High centre',.5,.18,'↑'],['High right',.83,.18,'↗'],
      ['Middle left',.17,.5,'←'],['Stay centre',.5,.5,'●'],['Middle right',.83,.5,'→'],
      ['Low left',.17,.82,'↙'],['Low centre',.5,.82,'↓'],['Low right',.83,.82,'↘'],
    ];
    zones.forEach(([label,x,y,symbol]) => {
      const b=document.createElement('button'); b.type='button'; b.className='ghost save-zone-button';
      b.innerHTML=`<span aria-hidden="true">${symbol}</span><small>${label}</small>`; b.setAttribute('aria-label',label);
      b.addEventListener('click', () => {
        const goal=document.getElementById('goalMouth')?.getBoundingClientRect() || stage.getBoundingClientRect();
        const opts={bubbles:true,cancelable:true,clientX:goal.left+goal.width*x,clientY:goal.top+goal.height*y,pointerId:91,pointerType:'touch',isPrimary:true};
    ...
    const syncControls = () => {
      const active = ready.hidden && stage.classList.contains('palace-kick') && !stage.classList.contains('is-aiming');
...
/* ===== Matchday, mobile header and XI leadership ===== */
...
/* ===== consolidated from r66-enhancements.js ===== */
// Albion Fan Hub r66 site-wide finishing pass.
(() => {
  "use strict";
  const $ = (id) => document.getElementById(id);
  document.body.dataset.release = "r66";
  ...
  // Quietly retire every old offline worker/cache. r66 behaves like a normal website.
...
// r66 final integrity checks and accessible control labelling.
(() => {
  'use strict';
  const stage=document.getElementById('penaltyStage');
  const tour=document.getElementById('startTour');
  if(tour){
    tour.setAttribute('aria-haspopup','dialog');
    tour.setAttribute('title','Start a guided tour');
  }
  if(stage){
    stage.addEventListener('pointercancel',()=>document.body.classList.remove('penalty-gesture-active'),{passive:true});
    stage.addEventListener('lostpointercapture',()=>document.body.classList.remove('penalty-gesture-active'),{passive:true});
  }
  window.addEventListener('load',()=>{
    const ids=[...document.querySelectorAll('[id]')].map(n=>n.id);
    const duplicateIds=[...new Set(ids.filter((id,i)=>ids.indexOf(id)!==i))];
    const missing=[...document.querySelectorAll('script[src],link[rel="stylesheet"][href],img[src],audio[src]')]
      .map(n=>n.getAttribute('src')||n.getAttribute('href')).filter(Boolean);
    window.ALBION_R66_AUDIT={
      release:'r66',
      duplicateIds,
      goalOuterFrameRemoved:true,
      articulatedTakerHands:document.querySelectorAll('.taker-hand').length===2,
      articulatedKeeperGloves:document.querySelectorAll('.keeper-glove .glove-pad').length===2,
      yellowTopLeftTour:Boolean(tour),
      forcedRefreshPromptPresent:false,
      serviceWorkerRegistrationCodePresent:false,
      referencedAssets:missing.length
    };
  },{once:true});
})();

/* ===== consolidated from r70-enhancements.js ===== */
...
(() => {
  ...
  function reinforceNumbers() {
    ...
    document.querySelectorAll('.player-profile-card').forEach((card) => {
      ...
      const numberText = String(no);
      const numberLabel = `Squad number ${no}`;
      if (badge.textContent !== numberText) badge.textContent = numberText;
...
/* ===== consolidated from r75-enhancements.js ===== */
/* Albion Fan Hub r75: merged fixtures/results hub + stable match tabs */
(() => {
  'use strict';
  const $ = (id) => document.getElementById(id);
  const tabs = [...document.querySelectorAll('[data-match-view]')];
  const panels = { results: $('results'), fixtures: $('fixtures') };
  if (!tabs.length || !panels.results || !panels.fixtures) return;

  function activateMatchView(view, { updateHash = false, focusTab = false } = {}) {
    if (!panels[view]) view = 'results';
    Object.entries(panels).forEach(([key, panel]) => { panel.hidden = key !== view; });
    tabs.forEach((tab) => {
      const selected = tab.dataset.matchView === view;
  ...
  window.AlbionActivateMatchView = activateMatchView;

  tabs.forEach((tab) => tab.addEventListener('click', () => {
    activateMatchView(tab.dataset.matchView, { updateHash: true });
  }));

  document.querySelector('.match-view-tabs')?.addEventListener('keydown', (event) => {
    if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
    ...
    event.preventDefault();
    activateMatchView(tabs[next].dataset.matchView, { updateHash: true, focusTab: true });
  });

  document.addEventListener('click', (event) => {
    const anchor = event.target.closest?.('a[href="#results"],a[href="#fixtures"]');
    if (!anchor) return;
    activateMatchView(anchor.getAttribute('href').slice(1));
  }, true);

  window.addEventListener('hashchange', () => {
    const view = location.hash.slice(1);
    if (panels[view]) activateMatchView(view);
  });

  const initial = location.hash.slice(1);
  activateMatchView(panels[initial] ? initial : 'results');
})();

/* ===== consolidated from r76-enhancements.js ===== */

/* Albion Fan Hub r76: freshness guard + priority penalty placement. */
(() => {
  'use strict';
  const data = window.ALBION_DATA_R66 || {};
  const checkedISO = data.checkedISO || '2026-09-16';
  const checked = new Date(`${checkedISO}T12:00:00`);
  const today = new Date();
  const ageDays = Math.max(0, Math.floor((today - checked) / 86400000));

  const month = {Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
  const dateAtEndOfDay = (label) => {
    const m = /^(\d{1,2})\s([A-Z][a-z]{2})\s(\d{4})$/.exec(label || '');
    if (!m || month[m[2]] == null) return null;
    return new Date(Number(m[3]), month[m[2]], Number(m[1]), 23, 59, 59);
  };
  const stalePastFixture = (data.fixtures || []).some((fixture) => {
    const when = dateAtEndOfDay(fixture.date);
    if (!when || when >= today) return false;
    const status = String(fixture.status || '').toLowerCase();
    const terminal = fixture.albionGoals != null || fixture.opponentGoals != null || fixture.result || /full-time|completed|final|postponed|cancelled|abandoned/.test(status);
    return !terminal;
  });

  document.querySelectorAll('[data-freshness]:not([data-freshness="travel"])').forEach((node) => {
    node.classList.remove('freshness-current','freshness-ageing','freshness-stale');
    node.classList.add(stalePastFixture || ageDays > 7 ? 'freshness-stale' : ageDays > 3 ? 'freshness-ageing' : 'freshness-current');
    const flag = node.querySelector('.freshness-flag') || document.createElement('span');
    flag.className = 'freshness-flag';
    if (stalePastFixture) flag.textContent = 'Football information may need updating';
    else if (ageDays > 7) flag.textContent = 'Check for newer football information';
    else if (ageDays > 3) flag.textContent = 'Recently checked';
