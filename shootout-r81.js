// Albion Fan Hub r81 penalty game — explicit pre-whistle dive choice + mobile keeper scale
(() => {
  "use strict";
  ...
  function resetVisuals() {
    ...
    if (preWhistleControls) preWhistleControls.hidden = true;
  ...
  function recoverKeeperAfterDive(token, saved = false) {
    window.setTimeout(() => {
      ...
      if (state.pendingDive && ["palace-prewhistle", "palace-run"].includes(state.phase)) return;
  ...
  async function beginPalacePenalty() {
    ...
    stage.classList.add("palace-kick", "is-locked");
    const player = takerAt("palace", state.palaceKicks);
    applyTakerPose(player, state.palaceKicks + 20);
  ...
  stage.addEventListener("pointerdown", (event) => {
    if (event.target.closest?.(".pre-whistle-controls")) return;
    unlockAudio();
    const touchLike = event.pointerType === "touch" || event.pointerType === "pen";
    const point = eventGoalPoint(event, touchLike ? AIM_MARGIN : AIM_MARGIN);
  ...
  stage.addEventListener("touchstart", (event) => {
    ...
    const savingPhase = state.phase === "save";
    const earlySavingPhase = state.phase === "palace-prewhistle" || state.phase === "palace-run";
