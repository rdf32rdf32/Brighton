// Albion Fan Hub r80 penalty game — pre-whistle goalkeeper dives restored
(() => {
  "use strict";
  ...
  // The centre of the rendered 18-yard line is y=570 in a 650-high pitch SVG.
  // Starting boots are normally 1.2–4.5% of the stage beyond that line.
  const PENALTY_AREA_LINE_Y = 570 / 650;

  function mobileRunUpBootY(profile = state.pendingRunProfile || "angle", style = state.pendingRunStyle || "direct") {
    const byProfile = { angle: .922, straight: .907, reverse: .917, stutter: .889 };
    let bootY = byProfile[profile] ?? .915;
    if (style === "quick") bootY -= .012;
    if (style === "measured") bootY += .004;
    return clamp(bootY, PENALTY_AREA_LINE_Y + .007, .932);
  }

  function mobileReadyTopPx(profile = state.pendingRunProfile, style = state.pendingRunStyle) {
    const stageHeight = Math.max(1, stage.clientHeight);
    const renderedHeight = taker.getBoundingClientRect().height || stageHeight * .18;
    const desiredBootY = mobileRunUpBootY(profile, style);
    return clamp(stageHeight * desiredBootY - renderedHeight * takerBootRatio, stageHeight * .48, stageHeight * .82);
  }

  function mobileReadyLeftPercent(profile = state.pendingRunProfile || "angle") {
    const leftFooted = taker.dataset.foot === "left";
    const positions = {
      angle: leftFooted ? 55.5 : 44.5,
      straight: leftFooted ? 53.2 : 46.8,
      reverse: leftFooted ? 44.8 : 55.2,
      stutter: leftFooted ? 53 : 47,
    };
    return positions[profile] ?? (leftFooted ? 54 : 46);
  }

  function setTakerReadyPosition(profile = state.pendingRunProfile, style = state.pendingRunStyle) {
    const mobile = mobilePenaltyLayout();
    const readyLeft = mobileReadyLeftPercent(profile);
    const readyTop = mobileReadyTopPx(profile, style);
    taker.style.left = mobile ? `${readyLeft}%` : "50%";
    taker.style.top = mobile ? `${readyTop}px` : "49%";
    taker.style.transform = "translate(-50%,0)";
    stage.dataset.takerReady = mobile ? "outside-box" : "desktop";
    if (mobile) {
      stage.style.setProperty("--run-start-boot-y", `${mobileRunUpBootY(profile, style) * 100}%`);
      stage.style.setProperty("--run-start-left", `${readyLeft}%`);
    } else {
      stage.style.removeProperty("--run-start-boot-y");
      stage.style.removeProperty("--run-start-left");
    }
  }
  ...
  // Keep the opening pair cinematic, then tighten repeated routines so a full shoot-out moves briskly.
  const ceremonyPace = () => {
    const kicks = (state?.albionKicks || 0) + (state?.palaceKicks || 0);
    return kicks < 2 ? 1 : kicks < 6 ? 0.68 : 0.60;
  };
  ...
  const albionTakers = window.ALBION_DATA_R66?.penaltyTakers || [];
  const palaceTakers = window.ALBION_DATA_R66?.palaceTakers || [];
  ...
  const state = {
    ...
    ceremonySkip: false,
    pendingRunProfile: "angle",
    pendingRunStyle: "direct",
    albionOrder: [],
    palaceOrder: [],
  };
  ...
  function saveFeedbackText(saved,saveType=""){
    ...
    const rawMs=Math.round(state.userDive.timing);
    const timing=rawMs<0?Math.abs(rawMs)+" ms early":rawMs+" ms";
    const score=Number(state.userDive.reachScore);
    const precision=Number.isFinite(score)?score<=.48?"well positioned":score<=1?"full stretch":"just beyond reach":state.userDive.zone;
    const direction=Number(state.userDive.directionQuality);
    const read=Number.isFinite(direction)?direction>.92?"direction read":direction>.68?"partial read":"wrong read":"";
    return saved?timing+" · "+[read,precision,String(saveType||"save").toLowerCase()].filter(Boolean).join(" · "):timing+" · "+[read,precision].filter(Boolean).join(" · ");
  }
  ...
  function resetVisuals() {
    ...
    stage.classList.remove("palace-kick", "pre-whistle-save-window", "is-save-window", "is-locked", "is-waiting", "is-aiming", "is-drag-aiming", "placing-ball");
  ...
  function resolveKeeperSave(target,dive,shot){
    if(!dive)return{saved:false,saveType:"",distance:Infinity,reach:0,edge:false,reachScore:Infinity,timingQuality:0,directionQuality:0};
    const settings=config(),timing=dive.timing,reactionWindow=shot.reactionWindow;
    const rawX=Number.isFinite(dive.rawX)?dive.rawX:dive.x,rawY=Number.isFinite(dive.rawY)?dive.rawY:dive.y;
    const targetCentre=Math.abs(target.x-.5)<.135,diveCentre=Math.abs(rawX-.5)<.16;
    ...
    const targetSide=target.x<.40?-1:target.x>.60?1:0,diveSide=rawX<.40?-1:rawX>.60?1:0;
    const targetHeight=target.y<.34?-1:target.y>.67?1:0,diveHeight=rawY<.34?-1:rawY>.67?1:0;
    const sameSide=targetSide===diveSide,oppositeSide=targetSide!==0&&diveSide!==0&&targetSide!==diveSide;
    const heightGap=Math.abs(targetHeight-diveHeight);
    const sideRead=oppositeSide?.42:sameSide?1:(targetCentre&&diveCentre?1:.74);
    const heightRead=heightGap===0?1:heightGap===1?.82:.58;
    const directionQuality=clamp(sideRead*heightRead,.24,1);
    ...
    const anticipationBoost=timing< -90&&timing> -560&&sameSide?1.045:1;
    const cleanReachX=Math.max(.085,settings.saveRadius*timingFactor*powerFactor*sideFactor*sideRead*anticipationBoost);
    const cleanReachY=Math.max(.08,settings.saveRadius*.92*timingFactor*powerFactor*heightFactor*heightRead);
    const tipBonus=settings.fingertipMargin*clamp(timingFactor,.72,1.06);
    const tipReachX=cleanReachX+tipBonus*(sameSide?1:.68);
    const tipReachY=cleanReachY+tipBonus*.82;

    const dx=Math.abs(target.x-dive.x),dy=Math.abs(target.y-dive.y);
    const cleanScore=Math.hypot(dx/cleanReachX,dy/cleanReachY);
    const fingertipScore=Math.hypot(dx/tipReachX,dy/tipReachY);
    const distance=Math.hypot(target.x-dive.x,(target.y-dive.y)*.9);
    const cleanReach=Math.sqrt(cleanReachX*cleanReachY);
    const fingertipReach=Math.sqrt(tipReachX*tipReachY);
    ...
    const saved=centralHold||bodyBlock||fingertipScore<=1;

    dive.timingQuality=timingFactor;
    dive.directionQuality=directionQuality;
    dive.reachScore=fingertipScore;

    if(!saved)return{saved:false,saveType:"",distance,reach:fingertipReach,edge:false,reachScore:fingertipScore,timingQuality:timingFactor,directionQuality};
    const edge=!centralHold&&!bodyBlock&&cleanScore>1;
    let saveType="";
    ...
    else if(cleanScore<.46&&timing>=-120&&timing<270&&shot.power<.86)saveType="CATCH";
    else if(bodyBlock)saveType="BLOCKED";
    else saveType="PARRIED";
    return{saved:true,saveType,distance,reach:edge?fingertipReach:cleanReach,edge,reachScore:fingertipScore,timingQuality:timingFactor,directionQuality};
  }
  ...
  function animateRunUp(isPalace, foot = "right", target = null, style = "direct", profile = chooseRunUpProfile(foot, style)) {
    ...
    const mobileBase = isPalace ? Math.round(1220 * settings.runUpScale) : style === "quick" ? 980 : style === "measured" ? 1220 : style === "stutter" ? 1280 : 1130;
    ...
    if (mobileRun) {
      ...
      if (bootRect?.height && ballRect?.height) {
        ...
        contactLift = Math.round(clamp(ballContactY - bootContactY, -stage.clientHeight * .29, 10));
      } else {
        contactLift = -58;
      }
    }
    const runStartY = mobileRun ? 0 : 20;
    const runMidY = mobileRun ? Math.round(contactLift * .31) : 7;
    const runNearY = mobileRun ? Math.round(contactLift * .68) : -10;
    const followLift = contactLift - (mobileRun ? 8 : 4);
    const startScale = mobileRun ? (style === "quick" || profile === "stutter" ? .935 : .91) : .965;
    const earlyScale = mobileRun ? .95 : .978;
    ...
    const runFrames = stutter ? [
      { transform: `translate(calc(-50% + ${startX}px),${runStartY}px) scale(${startScale})` },
      { transform: `translate(calc(-50% + ${startX * .76}px),${Math.round(contactLift * .12)}px) scale(${earlyScale})`, offset: .16 },
      { transform: `translate(calc(-50% + ${startX * .55}px),${Math.round(contactLift * .26)}px) scale(.972)`, offset: .32 },
      { transform: `translate(calc(-50% + ${startX * .42}px),${runMidY}px) scale(.985)`, offset: .46 },
      { transform: `translate(calc(-50% + ${startX * .38}px),${runMidY}px) scale(.985)`, offset: .58 },
      { transform: `translate(calc(-50% + ${footDirection * 5}px),${runNearY}px) scale(1.005)`, offset: .76 },
      { transform: `translate(calc(-50% + ${contactOffset}px),${contactLift}px) scale(1.018)`, offset: .91 },
      { transform: `translate(calc(-50% + ${contactOffset + footDirection * 11}px),${followLift}px) scale(1.005)` },
    ] : [
      { transform: `translate(calc(-50% + ${startX}px),${runStartY}px) scale(${startScale})` },
      { transform: `translate(calc(-50% + ${startX * (reverse ? .84 : .76)}px),${Math.round(contactLift * .13)}px) scale(${earlyScale})`, offset: .14 },
      { transform: `translate(calc(-50% + ${startX * (reverse ? .64 : .54)}px),${Math.round(contactLift * .29)}px) scale(.972)`, offset: .29 },
      { transform: `translate(calc(-50% + ${startX * (reverse ? .42 : .28)}px),${Math.round(contactLift * .48)}px) scale(.988)`, offset: .47 },
      { transform: `translate(calc(-50% + ${footDirection * (reverse ? 10 : 6)}px),${runNearY}px) scale(1.005)`, offset: .66 },
      { transform: `translate(calc(-50% + ${contactOffset * .35}px),${Math.round(contactLift * .86)}px) scale(1.018)`, offset: .8 },
      { transform: `translate(calc(-50% + ${contactOffset}px),${contactLift}px) scale(1.018)`, offset: .91 },
    ...
    if (mobileFlight) {
      ...
      if (ballShadow) {
        const shadowFrames = path.map((k, i) => {
          ...
          const opacity = i === path.length - 1 ? (saved ? .14 : .05) : Math.max(.08, .58 * (1 - k.o * .92));
          return {
  ...
  function animateKeeperDive(point, duration = 840, saved = false) {
    ...
    const high = point.y < .43;
    const low = point.y > .64;
    ...
    const totalDuration = Math.max(duration, extreme ? 900 : wide ? 835 : 770);

    // Push leg: compress first, then extend sharply through the ground.
    if (pushLeg) animateElement(pushLeg, [
      { transform: "rotate(0deg) translateY(0)" },
      { transform: `rotate(${direction * -15}deg) translateY(5px)`, offset: .13 },
      { transform: `rotate(${direction * -5}deg) translateY(3px)`, offset: .22 },
      { transform: `rotate(${direction * 20}deg) translateY(0)`, offset: .39 },
      { transform: `rotate(${direction * 30}deg) translateY(-2px)`, offset: .68 },
      { transform: `rotate(${direction * 22}deg)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.14,.7,.18,1)" });

    if (pushLowerLeg) animateElement(pushLowerLeg, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${direction * -28}deg)`, offset: .13 },
      { transform: `rotate(${direction * -18}deg)`, offset: .22 },
      { transform: `rotate(${direction * 10}deg)`, offset: .42 },
      { transform: `rotate(${direction * 20}deg)`, offset: .72 },
      { transform: `rotate(${direction * 14}deg)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.2,.62,.2,1)" });

    // Trail leg hangs back in flight instead of rotating in lockstep with the torso.
    if (trailLeg) animateElement(trailLeg, [
      { transform: "rotate(0deg) translateY(0)" },
      { transform: `rotate(${direction * 7}deg) translateY(2px)`, offset: .16 },
      { transform: `rotate(${direction * -10}deg) translateY(0)`, offset: .4 },
      { transform: `rotate(${direction * -34}deg) translateY(1px)`, offset: .72 },
      { transform: `rotate(${direction * -27}deg) translateY(3px)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.16,.62,.2,1)" });

    if (trailLowerLeg) animateElement(trailLowerLeg, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${direction * 8}deg)`, offset: .24 },
      { transform: `rotate(${direction * 20}deg)`, offset: .5 },
      { transform: `rotate(${direction * 29}deg)`, offset: .75 },
      { transform: `rotate(${direction * 22}deg)` },
    ], { duration: totalDuration });

    // Near hand leads the dive. The elbow opens progressively rather than snapping straight.
    if (leadingArm) animateElement(leadingArm, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${leadAngle * .12}deg)`, offset: .16 },
      { transform: `rotate(${leadAngle * .42}deg)`, offset: .34 },
      { transform: `rotate(${leadAngle * .78}deg)`, offset: .58 },
      { transform: `rotate(${leadAngle}deg)`, offset: .82 },
      { transform: `rotate(${leadAngle * .98}deg)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.12,.7,.16,1)" });

    if (leadingLowerArm) animateElement(leadingLowerArm, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${direction * 17}deg)`, offset: .22 },
      { transform: `rotate(${direction * 10}deg)`, offset: .48 },
      { transform: `rotate(${direction * 3}deg)`, offset: .78 },
      { transform: "rotate(0deg)" },
    ], { duration: totalDuration, easing: "cubic-bezier(.18,.7,.2,1)" });

    // Far hand follows for balance and, on catches, support.
    if (trailingArm) animateElement(trailingArm, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${trailAngle * .12}deg)`, offset: .25 },
      { transform: `rotate(${trailAngle * .44}deg)`, offset: .48 },
      { transform: `rotate(${trailAngle * .82}deg)`, offset: .74 },
      { transform: `rotate(${trailAngle}deg)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.16,.64,.2,1)" });

    if (trailingLowerArm) animateElement(trailingLowerArm, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${direction * -5}deg)`, offset: .4 },
      { transform: `rotate(${direction * -11}deg)`, offset: .68 },
      { transform: `rotate(${direction * -5}deg)` },
    ], { duration: totalDuration });

    // Torso loads against the dive, then the shoulder leads while hips lag slightly.
    if (body) animateElement(body, [
      { transform: "translate(0,0) rotate(0deg) scale(1)" },
      { transform: `translate(${counterLoad * 2.5}px,4px) rotate(${counterLoad * 2}deg) scale(1,.95)`, offset: .13 },
      { transform: `translate(${direction * 1}px,1px) rotate(${direction * 2}deg) scale(1.01,.99)`, offset: .27 },
      { transform: `translate(${direction * 4}px,-2px) rotate(${direction * 8}deg) scale(1.025,1)`, offset: .54 },
      { transform: `translate(${direction * 5}px,-1px) rotate(${direction * 11}deg) scale(1.02,.995)`, offset: .8 },
      { transform: `translate(${direction * 4}px,4px) rotate(${direction * 9}deg) scale(1,.97)` },
    ], { duration: totalDuration, easing: "cubic-bezier(.14,.66,.18,1)" });

    // Keep the eyes closer to level than the body: goalkeepers track the ball in flight.
    if (head) animateElement(head, [
      { transform: "rotate(0deg)" },
      { transform: `rotate(${direction * 2}deg)`, offset: .2 },
      { transform: `rotate(${direction * -4}deg)`, offset: .48 },
      { transform: `rotate(${direction * -10}deg)`, offset: .82 },
    ...
    const diveAnimation = animateElement(keeper, [
      // Set/load: a tiny movement away from the dive stores force in the outside leg.
  ...
  function recoverKeeperAfterDive(token, saved = false) {
    window.setTimeout(() => {
      ...
      const recovery = animateElement(keeper, [
        ...
        // Absorb the landing before rolling the hips back under the shoulders.
        { transform: "translateX(-50%) translateY(7px) rotate(0deg) scale(1,.965)", offset: .34 },
        { transform: "translateX(-50%) translateY(4px) rotate(0deg) scale(.995,.985)", offset: .58 },
        // Small rise into the ready stance rather than snapping upright.
  ...
  async function beginPalacePenalty() {
    ...
    $("stageInstruction").textContent = "Gamble now — or wait for the whistle";
    setStatus("You are Verbruggen", "You can commit to a dive immediately, or wait for the whistle and read the run-up.");
    reticle.hidden = true;
    ...
    stage.classList.remove("is-waiting", "is-locked");
    stage.classList.add("palace-kick", "pre-whistle-save-window");
    const player = takerAt("palace", state.palaceKicks);
    applyTakerPose(player, state.palaceKicks + 20);
    ...
    state.palaceShots.push({ x: state.palaceTarget.x, y: state.palaceTarget.y, result: palaceOutcome, name: player.name, saveType, power: state.currentShotPower, reactionWindow: state.currentReactionWindow, reactionMs: state.userDive ? Math.round(state.userDive.timing) : null, dive: state.userDive ? { x: state.userDive.x, y: state.userDive.y } : null, reachScore: Number.isFinite(state.userDive?.reachScore) ? Number(state.userDive.reachScore.toFixed(3)) : null, directionQuality: Number.isFinite(state.userDive?.directionQuality) ? Number(state.userDive.directionQuality.toFixed(3)) : null });
    refereeSignal(scored);
  ...
  function queueEarlyDive(point, source = "early-gamble") {
    if (!["palace-prep", "palace-run"].includes(state.phase) || state.pendingDive || state.userDive) return;
  ...
  stage.addEventListener("pointerup", (event) => {
    ...
    if (state.phase === "palace-prep" || state.phase === "palace-run") {
      ...
      if (!state.pendingDive && hadTracking) {
        ...
        else if (!isCentreSavePoint(fallbackPoint) && distance < Math.max(8, saveThreshold * .45)) queueEarlyDive(fallbackPoint, touchLike ? "early-tap" : "early-click");
        else {
          $("stageInstruction").textContent = "Holding centre — wait for React now";
          setStatus("Holding position", "No early commitment. Stay still for a central shot or react at contact.");
        }
      }
      ...
      if (touchLike && distance >= saveThreshold) {
        ...
        $("stageInstruction").textContent = "Swipe further, or stay still for centre";
        setStatus("No side committed", "A short uncertain swipe is ignored. Staying still keeps Verbruggen central.");
      } else if (isCentreSavePoint(fallbackPoint)) {
        $("stageInstruction").textContent = "Stay still for centre";
        setStatus("Holding centre", "No input is needed for a central body save.");
      }
    }
  ...
  stage.addEventListener("touchstart", (event) => {
    ...
    const savingPhase = state.phase === "save";
    const earlySavingPhase = state.phase === "palace-prep" || state.phase === "palace-run";
    if (state.phase !== "albion-aim" && !savingPhase && !earlySavingPhase) return;
    ...
    if (state.phase === "albion-aim") setReticle(point.x, point.y);
      ...
      if (state.reactionOpen && isCentreSavePoint(tapPoint)) beginCentreHold(tapPoint);
      if (earlySavingPhase) {
        $("stageInstruction").textContent = "Swipe or tap a side to gamble now";
        setStatus("Pre-whistle gamble available", "Commit now, or keep still and wait for the whistle.");
      }
    }
    event.preventDefault();
  }, { passive: false });
  ...
  stage.addEventListener("touchmove", (event) => {
    ...
    } else if ((state.phase === "palace-prep" || state.phase === "palace-run") && !state.pendingDive) {
      const distance = Math.hypot(touch.clientX - fallbackTouchStart.clientX, touch.clientY - fallbackTouchStart.clientY);
      const mapped = distance >= 18 ? swipeGoalPoint(fallbackTouchStart, touch, mobileTapGoalPoint(touch)) : mobileTapGoalPoint(touch);
  ...
  stage.addEventListener("keydown", (event) => {
    ...
    if (event.key === "ArrowLeft") setReticle(state.aim.x - step, state.aim.y);
    ...
    else if (["save","palace-prep","palace-run"].includes(state.phase) && ["1","2","3","4","5","6","7","8","9"].includes(event.key)) {
      ...
      else queueEarlyDive(point,"early-keyboard");
