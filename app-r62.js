/* ===== Albion Fan Hub r62 application bundle ===== */
window.ALBION_CONTENT = {
  ...
  squad: window.ALBION_DATA_R62.squad.map(({ name, position }) => ({ name, position })),
  playerProfiles: window.ALBION_DATA_R62.squad.map(({ name, position, role, number, nationality, initials, summary }) => ({ name, position, role, number, nationality, initials, summary })),
  fixtures: [
    {
      date: "23 Aug 2026",
...
(() => {
  ...
  // r62: one fixture list drives the hero, countdown, sticky bar, Match Centre and predictor.
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => navigator.serviceWorker.register("service-worker.js?v=r62").catch(() => {}), { once: true });
  }
})();
...
(() => {
  ...
  function diagnostics() {
    const run = () => {
      ...
      const activeNames = (window.ALBION_DATA_R62?.squad || []).filter((player) => player.active !== false).map((player) => player.name);
      const duplicates = activeNames.filter((name, index) => activeNames.indexOf(name) !== index);
      const penaltyNames = window.ALBION_DATA_R62?.penaltyTakers?.map((player) => player.name) || [];
...
/* ===== r62 matchday, mobile header and XI leadership ===== */
