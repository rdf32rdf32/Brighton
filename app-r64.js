/* ===== Albion Fan Hub r64 application bundle ===== */
window.ALBION_CONTENT = {
  ...
  squad: window.ALBION_DATA_R64.squad.map(({ name, position }) => ({ name, position })),
  playerProfiles: window.ALBION_DATA_R64.squad.map(({ name, position, role, number, nationality, initials, summary }) => ({ name, position, role, number, nationality, initials, summary })),
  fixtures: [
    {
      date: "23 Aug 2026",
...
(() => {
  ...
  // r64: one fixture list drives the hero, countdown, sticky bar, Match Centre and predictor.
...
(() => {
  ...
  function diagnostics() {
    const run = () => {
      ...
      const penaltyNames = window.ALBION_DATA_R64?.penaltyTakers?.map((player) => player.name) || [];
