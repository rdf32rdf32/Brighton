/* ===== Albion Fan Hub r63 application bundle ===== */
window.ALBION_CONTENT = {
  ...
  freshness: {
    fixtures: "4 August 2026",
    squad: "4 August 2026",
    travel: "4 August 2026",
    history: "4 August 2026",
  },
  squad: window.ALBION_DATA_R63.squad.map(({ name, position }) => ({ name, position })),
  playerProfiles: window.ALBION_DATA_R63.squad.map(({ name, position, role, number, nationality, initials, summary }) => ({ name, position, role, number, nationality, initials, summary })),
  fixtures: [
    {
      date: "23 Aug 2026",
...
(() => {
  ...
  // r63: one fixture list drives the hero, countdown, sticky bar, Match Centre and predictor.
...
(() => {
  ...
  function diagnostics() {
    const run = () => {
      ...
      const activeNames = (window.ALBION_DATA_R63?.squad || []).filter((player) => player.active !== false).map((player) => player.name);
      const duplicates = activeNames.filter((name, index) => activeNames.indexOf(name) !== index);
      const penaltyNames = window.ALBION_DATA_R63?.penaltyTakers?.map((player) => player.name) || [];
